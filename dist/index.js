...
var require_build_custom_worker = __commonJS({
  "build-custom-worker.js"(exports2, module2) {
    "use strict";
    var path2 = require("path");
    var fs2 = require("fs");
    var webpack = require("webpack");
    var { CleanWebpackPlugin: CleanWebpackPlugin2 } = require("clean-webpack-plugin");
    var TerserPlugin = require("terser-webpack-plugin");
    var buildCustomWorker2 = ({ id, basedir, customWorkerDir, destdir, plugins, minify }) => {
      let workerDir = void 0;
      if (fs2.existsSync(path2.join(basedir, customWorkerDir))) {
        workerDir = path2.join(basedir, customWorkerDir);
      } else if (fs2.existsSync(path2.join(basedir, "src", customWorkerDir))) {
        workerDir = path2.join(basedir, "src", customWorkerDir);
      }
      if (!workerDir) return;
      const name = `worker-${id}.js`;
      const customWorkerEntries = ["ts", "js"].map((ext) => path2.join(workerDir, `index.${ext}`)).filter((entry) => fs2.existsSync(entry));
      ...
      console.log(`> [PWA] Build custom worker: ${path2.join(destdir, name)}`);
      webpack({
        mode: "none",
        target: "webworker",
        entry: {
        ...
        plugins: [
          new CleanWebpackPlugin2({
            cleanOnceBeforeBuildPatterns: [path2.join(destdir, "worker-*.js"), path2.join(destdir, "worker-*.js.map")]
          })
        ].concat(plugins),
        optimization: minify ? {
          minimize: true,
    ...
    module2.exports = buildCustomWorker2;
  }
});

// build-fallback-worker.js
var require_build_fallback_worker = __commonJS({
  "build-fallback-worker.js"(exports2, module2) {
    "use strict";
    var path2 = require("path");
    var fs2 = require("fs");
    var webpack = require("webpack");
    ...
    var getFallbackEnvs = ({ fallbacks, basedir, id, pageExtensions }) => {
      ...
      if (!document) {
        let pagesDir = void 0;
        if (fs2.existsSync(path2.join(basedir, "pages"))) {
          pagesDir = path2.join(basedir, "pages");
        } else if (fs2.existsSync(path2.join(basedir, "src", "pages"))) {
          pagesDir = path2.join(basedir, "src", "pages");
        }
        if (!pagesDir) return;
        const offlines = pageExtensions.map((ext) => path2.join(pagesDir, `_offline.${ext}`)).filter((entry) => fs2.existsSync(entry));
        if (offlines.length === 1) {
          document = "/_offline";
        }
      }
      if (data && data.endsWith(".json")) {
        data = path2.posix.join("/_next/data", id, data);
      }
      const envs = {
    ...
    var buildFallbackWorker2 = ({ id, fallbacks, basedir, destdir, minify, pageExtensions }) => {
      const envs = getFallbackEnvs({ fallbacks, basedir, id, pageExtensions });
      if (!envs) return;
      const name = `fallback-${id}.js`;
      const fallbackJs = path2.join(__dirname, `fallback.js`);
      webpack({
        mode: "none",
        target: "webworker",
        entry: {
        ...
        plugins: [
          new CleanWebpackPlugin2({
            cleanOnceBeforeBuildPatterns: [path2.join(destdir, "fallback-*.js"), path2.join(destdir, "fallback-*.js.map")]
          }),
    ...
    module2.exports = buildFallbackWorker2;
  }
});
